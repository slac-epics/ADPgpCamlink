...
{
public:
	//* this isn't really the raw data anymore. it's cast as a uint8_t now.
	std::vector<uint8_t>	raw_data;
	std::vector<uint8_t>    main_header;   
	//* the size of each subframe extracted from the tail
	std::vector<uint16_t>   frame_sizes_reverse_order;
	
	//* vector of vectors.
	//* N elements long with each element containing a start and an end.
	std::vector<std::vector<uint16_t>>    frame_positions_reverse_order;

	//* The actual edge positions, camera images, time stamps, etc...
	//* will be elements of this array.  
	...
	//* This method looks at the position of the raw data
	//* indicated in the argument,
	//* process the data at that location,
	//* and returns the position of the next place to look
	//* for the next piece of data
	//* It is assumed that the position argument points to
	//* the section of the sub-frame tail that contains
	//* the subframe length information
	uint16_t frame_to_position(int position);
	int get_frame_size();

	//* This method loads the data into the parser class.
	//* May need to become a copy
