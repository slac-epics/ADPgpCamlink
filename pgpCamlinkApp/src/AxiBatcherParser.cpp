...
// This method looks at the position of the raw data indicated in the argument,
// processes the data at that location,
// and returns the position of the next place to look for the next piece of data.
// It is assumed that the position argument points to the section of the
// sub-frame tail that contains the subframe length information
...
int eventBuilderParser::parse_array()
{
    ...
    std::vector<uint16_t> sub_frame_range =
		{	uint16_t( raw_data.size() - spsft - frame_sizes_reverse_order.back() ),
			uint16_t( raw_data.size() - spsft )	};
    ...
    for (	std::vector<uint16_t>::iterator it = frame_sizes_reverse_order.begin();
			it != frame_sizes_reverse_order.end(); ++it	)
...
{
     // Bbefore populating for a sub batcher, we need to make sure the frames aren't.
	 // Otherwise there's a segfault in conditional declaration part
	 // frames[i].begin+2 that doesn't exist ( thanks gdb back trace for pointing here )
     // It's not clear where the damage that requires this check is coming from.  
     // It wasn't present when doing soft triggered testing at 75 KHz,
	 // so that indicates it's related to the timing and/or timestamp
...
template <class T> int eventBuilderParser::print_vector2d( std::vector<T> &my_vector )
{
    for ( uint32_t i = 0; i < my_vector.size(); i = i+1 )
	{
		...
		{
			printf( "subframe %d is a subbatcher \n",i );
		}
    }
    return 0;
}
